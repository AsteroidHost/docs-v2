...
# ⚡ Optimization Guide
This guide will show you how to optimize your server effectively! (20-30 minute read)

---

:::info Credits
This is a fantastic guide written by YouHaveTrouble on GitHub. Please click [**here**](https://github.com/YouHaveTrouble/minecraft-optimization) for the original post and [support the project](https://ko-fi.com/YouHaveTrouble).
This post has been modified to be accurate towards an Asteroid Host Server.
:::
...
It's key to remember that the overworld, nether and the end have separate world borders that need to be set up for each world. The nether dimension is 8x smaller than the overworld (if not modified with a datapack), so if you set the size wrong your players might end up outside of the world border! ⚠️
...
This is the distance in chunks (in a radius) that will be sent to players, similar to no-tick-view-distance from paper.
...
Keep in mind that the view distance is only the maximum range a player can view. This will not force a 12-chunk render distance on players with low end PCs.
...
This value overwrites the server.properties one if not set to `default`. You should keep it default to have both simulation and view distance in one place for easier management.
